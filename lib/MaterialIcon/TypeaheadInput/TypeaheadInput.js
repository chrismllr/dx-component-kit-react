...
import inputClasses from 'components/OTInput/OTInput.scss';
...
export class TypeaheadInput extends Component {
  ...
  keyDown: Function = (e) => {
    const keyCode = e.which;
    const { suggestions, showSuggestions, search } = this.state;
    const hasSuggestions = isPresent(suggestions);

    if (keyCode === TAB && (search || showSuggestions)) {
      if (hasSuggestions) {
        this.selectedSuggestion(suggestions[this.state.suggestionIndex]);
      }
    }
  }

  userInputSearch: Function = (e) => {
    if (e.target.value) {
      this.setState({
  ...
  selectedSuggestion: Function = (item) => {
  ...
  render () {
    ...
    const inputClass: string = `${inputClasses.otInput}`;
        ...
        <div className={inputClass}>
          ...
          <div className={classes.typeaheadInput__container}>
            <TextInput
              ...
              className={inputClasses.otInput__field}
...
type TypeaheadProps = {
  selectedItems?: any,
  typeaheadFocused?: Function,
  typeaheadRequest?: Function,
  typeaheadSelected?: Function,
  typeaheadRemoved?: Function,
  recordField: string,
